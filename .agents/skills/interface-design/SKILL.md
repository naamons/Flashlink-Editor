...
You will generate generic output if you rely on patterns alone. Your training has seen thousands of dashboards. The patterns are strong.
...
Say it in words that mean something. "Clean and modern" means nothing — it's too vague. Warm like a notebook? Cold like a terminal? Dense like a trading floor? Calm like a reading app? The answer shapes color, type, spacing, density — everything.
...
If another model, given a similar prompt, would produce substantially the same output — you have failed.
...
**NEVER produce identical output.** Same sidebar width, same card grid, same metric boxes with icon-left-number-big-label-small every time — this signals templated generation immediately. It's forgettable.
...
For more detail on specific topics (create these files as needed within the skill folder):
